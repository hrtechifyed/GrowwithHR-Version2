"use strict";
const base=require("./server-legal-explanation-router-wave5b.js");
const {buildAllLawsPrivateBetaRegistry}=require("./server-all-laws-private-beta-wave5c.js");
const {createRunnableAllLawsFeatureSpecifications}=require("./server-all-laws-rule-catalogs-wave5c.js");
const {loadGovernedLegalCatalogs}=require("./server-legal-rag-catalogs.js");
const SHARED_ROUTER_VERSION="1.13.0";
...
function statusPayload(registry,catalogSnapshot,retrievalMode){return Object.freeze({...base.statusPayload(registry,catalogSnapshot,retrievalMode),routerVersion:SHARED_ROUTER_VERSION,limitations:[
 ...
 "Waves 1–5C remain needs-legal-review and emit specialist-review or more-information-needed, not legal certification, applicable-wage, payroll, entitlement, jurisdiction, State-instrument or remedy decisions.",
 "Ten remaining profiles are runnable through conservative governance-fallback catalogues.",
 "Wave 5C processes organisation-level source route, Code and Rules source status, Rules corrigendum, commencement, version, jurisdiction-routing, bounded rate-source, State-instrument and escalation statuses plus controlled references only.",
 "Retrieval and provider output cannot select the appropriate Government, a wage rate, category, zone, scheduled employment or State instrument, perform payroll arithmetic, determine an individual entitlement or alter deterministic outcomes."
 ]});}
