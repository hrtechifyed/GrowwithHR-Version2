"use strict";
const base=require("./server-all-laws-private-beta-wave3c.js");
const {ESI_WAVE4A_CATALOG_ID,ESI_WAVE4A_CATALOG_PATH,ESI_WAVE4A_FEATURE_IDS,ESI_WAVE4A_PROFILE_DEFINITIONS}=require("./server-esi-wave4a-rule-catalogs.js");
...
function buildAllLawsPrivateBetaRegistry(options={}){
 ...
 const definitions=new Map(ESI_WAVE4A_PROFILE_DEFINITIONS.map((item)=>[item.featureId,item]));
 const featureIds=new Set(ESI_WAVE4A_FEATURE_IDS);
 ...
 registry.catalogs.push({catalogId:ESI_WAVE4A_CATALOG_ID,lawFamilyId:"esi",catalogPath:ESI_WAVE4A_CATALOG_PATH,format:"governed-legal-source-chunks-v1",runtimeStatus:"available-private-beta",legalReviewStatus:"needs-legal-review",catalogMode:"statutory",allowedFeatureIds:[...ESI_WAVE4A_FEATURE_IDS].sort()});
 registry.catalogs.sort((a,b)=>text(a.catalogId).localeCompare(text(b.catalogId)));
 registry.registryVersion="0.8.0";registry.updatedAt="2026-08-06";
 registry.limitations=[
  "Every registered profile is callable in private beta.",
  "Seven POSH, ten Maternity Benefit, twelve EPF/EPS/EDLI and five ESI Wave 4A employer-control profiles use feature-specific deterministic rules and governed catalogues.",
  ...
  "Wave 4A reviews establishment, employee-insurance, contractor, monthly-payment and accident-reporting controls without deciding ESI applicability, individual insurance, contribution amounts, accident causation or benefit entitlement.",
