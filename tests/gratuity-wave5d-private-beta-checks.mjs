...
import {createGratuityWave5dPayload,GRATUITY_WAVE5D_FEATURE_ID as BROWSER_FEATURE_ID} from "../js/assessment-v3/gratuity-wave5d-explanation-api-client.js";
const ROOT=path.resolve(path.dirname(fileURLToPath(import.meta.url)),"..");
const require=createRequire(import.meta.url);
const router=require(path.join(ROOT,"server-legal-explanation-router-wave5d.js"));
...
const {GRATUITY_WAVE5D_CATALOG_ID,GRATUITY_WAVE5D_FEATURE_IDS}=require(path.join(ROOT,"server-gratuity-wave5d-rule-catalogs.js"));
...
assert.equal(registry.profiles.filter((p)=>p.catalogId===GRATUITY_WAVE5D_CATALOG_ID).length,1);
assert.equal(registry.profiles.filter((p)=>p.catalogId===FALLBACK_CATALOG_ID).length,9);
...
const catalog=snapshot.catalogs[GRATUITY_WAVE5D_CATALOG_ID];
assert.equal(catalog.sources.length,4);
assert.equal(catalog.chunks.length,8);
...
assert.equal(status.substantiveProfileCount,48);
assert.equal(status.governanceFallbackProfileCount,9);
let scenarios=0;
for(const featureId of GRATUITY_WAVE5D_FEATURE_IDS){
 const profile=registry.profiles.find((p)=>p.featureId===featureId),spec=specifications[featureId];
 assert.ok(profile);assert.ok(spec);assert.equal(profile.catalogId,GRATUITY_WAVE5D_CATALOG_ID);
...
assert.equal(BROWSER_FEATURE_ID,"feature.legal.social-security.gratuity");
const payload=createGratuityWave5dPayload({answers:{
 gratuityDeclaredSourceRoute:"central-scope-candidate",gratuityChapterVSourceStatus:"evidenced",gratuityFirstScheduleSourceStatus:"evidenced",gratuityCentralRulesSourceStatus:"evidenced",gratuityCommencementTransitionStatus:"evidenced",gratuityLegacyTransitionControl:"evidenced",gratuityEstablishmentClassificationControl:"evidenced",gratuityWorkforceCategorySourceControl:"evidenced",gratuityAuthorityProcessSourceControl:"evidenced",gratuitySpecialistEscalationControl:"evidenced",gratuityEvidenceReferences:[{reference:"gratuity-source-readiness-register",body:"private evidence body"}],
 employeeName:"Private Person",nomineeName:"Private Nominee",heirName:"Private Heir",payrollBody:"private payroll",wageRecord:"private wage record",payslip:"private payslip",attendance:"private attendance",serviceRecord:"private service",claimBody:"private claim",disputeNarrative:"private dispute",noticeBody:"private notice",orderBody:"private order",medicalBody:"private medical",deathRecord:"private death",gratuityAmount:50000
}});
assert.deepEqual(payload,{answers:{gratuityDeclaredSourceRoute:"central-scope-candidate",gratuityChapterVSourceStatus:"evidenced",gratuityFirstScheduleSourceStatus:"evidenced",gratuityCentralRulesSourceStatus:"evidenced",gratuityCommencementTransitionStatus:"evidenced",gratuityLegacyTransitionControl:"evidenced",gratuityEstablishmentClassificationControl:"evidenced",gratuityWorkforceCategorySourceControl:"evidenced",gratuityAuthorityProcessSourceControl:"evidenced",gratuitySpecialistEscalationControl:"evidenced",gratuityEvidenceReferences:["gratuity-source-readiness-register"]}});
const payloadText=JSON.stringify(payload);
for(const prohibited of ["Private Person","Private Nominee","Private Heir","private payroll","private wage record","private payslip","private attendance","private service","private claim","private dispute","private notice","private order","private medical","private death","50000","private evidence body"])assert.equal(payloadText.includes(prohibited),false);
const manifest=require(path.join(ROOT,"growwithhr-rag/manifests/candidates/gratuity-source-pack.candidate.v1.json"));
const factContract=require(path.join(ROOT,"data/assessment/gratuity-assessment-fact-contract.v1.json"));
const sectionMap=require(path.join(ROOT,"data/legal-source-governance/gratuity-section-mapping.v1.json"));
const reviewDecision=require(path.join(ROOT,"data/legal-source-governance/gratuity-legal-review-decision.v1.json"));
...
assert.equal(factContract.providerPolicy.nomineeHeirDataAllowed,false);
assert.equal(sectionMap.features[0].approvalStatus,"not-approved");
assert.equal(reviewDecision.prohibitedOutcomes.includes("gratuity-amount-calculated"),true);
assert.equal(reviewDecision.prohibitedOutcomes.includes("employee-eligible"),true);
console.log(JSON.stringify({valid:true,profileCount:57,substantiveProfiles:48,substantiveGratuityWave5dProfiles:1,wave5dScenarios:scenarios,governanceFallbackProfiles:9,activeCatalogs:14,gratuitySources:4,gratuityChunks:8},null,2));
