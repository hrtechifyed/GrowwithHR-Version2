...
assert.ok(immediateBodyBootstrap || deferredReadyStateBootstrap,
...
assert.match(shellSource, /<div class="site-header-shell__inner">\s*<a class="site-brand-logo"[\s\S]*?<nav class="site-nav-glass"/,
...
assert.equal(styleImports.at(-1), "css/18-site-shell.css",
    "The static shared site shell remains last in styles.css; the runtime polish layer is loaded explicitly afterward.");
assert.ok(styleImports.indexOf("css/24-intelligence-hub.css") < styleImports.indexOf("css/18-site-shell.css"));
assert.equal(styleImports.filter((item) => item.includes("header-brand-lockup")).length, 0);
