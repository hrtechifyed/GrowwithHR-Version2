...
(() => {
    ...
    const VERSION = "1.0.0-report-identity";
    ...
    function mergeSource(payload = {}) {
        return Object.assign({}, payload, payload.lead || {}, payload.answers || {}, payload.report || {});
    }
    ...
    async function allocate(payload = {}) {
        ...
        activeAllocation = (async () => {
            ...
            if (!response.ok || !clean(result.reportId)) {
                throw new Error(result.error || "GrowWithHR could not reserve a unique report ID.");
            }
    ...
    window.GrowWithHRReportIdentity = Object.freeze({
        version: VERSION,
        endpoint: endpoint(),
        allocate
    });
