...
if (api && !document.documentElement.dataset.gwhChoiceExperience) {
  ...
  document.addEventListener('growwithhr:choice-help-opened', event => {
    const { control, entry, dialog } = event.detail;
  ...
  function renderOverview() {
    ...
    if (!form) return;
    const existing = form.querySelector('.gwh-choice-help-overview');
    ...
    const seen = new Set();
    controls().filter(control => form.contains(control)).forEach(control => {
    ...
    if (!seen.size) { existing?.remove(); return; }
    details.replaceChildren(summary, copy);
    if (!existing) intro.insertAdjacentElement('afterend', details);
  }
  ...
  const observer = new MutationObserver(records => {
    if (records.some(record => [...record.addedNodes, ...record.removedNodes].some(node => node.nodeType === 1 && (node.matches?.('select, input[type="radio"], input[type="checkbox"]') || node.querySelector?.('select, input[type="radio"], input[type="checkbox"]'))))) update();
  });
