...
(() => {
    ...
    const VERSION = "2.0.0-founder-web-report";
    ...
    async function waitForWebEngine() {
        for (let attempt = 0; attempt < 320; attempt += 1) {
            ...
            if (typeof pdf?.buildAdvisoryModel === "function" && typeof pdf?.buildReportLawTransparency === "function") return pdf;
    ...
    async function buildView() {
        ...
        try {
            ...
            const rows = Array.from(engine.buildReportLawTransparency(payload, model) || []);
            ...
            root.innerHTML = `<header class="gwh-web-hero"><img src="assets/hrtechify-logo.png" alt="HRTechify" class="gwh-web-logo"><div><p class="gwh-web-eyebrow">GROWWITHHR · RESEARCH PROTOTYPE</p><h1>HR Compliance & Growth Report</h1><p>${esc(clean(data.companyName, "Your Organisation"))}</p></div><button class="primary-btn" id="downloadFounderReport">Download PDF</button></header>
                ...
                ${missing.length ? `<section class="gwh-web-section"><p class="gwh-web-eyebrow">INFORMATION THAT COULD CHANGE THIS REPORT</p><h2>Complete the picture</h2><p class="gwh-web-lede">GrowWithHR does not guess missing company facts. Update these details in the assessment and generate a fresh report.</p>${missing.map((row) => `<article class="gwh-web-simple"><div class="gwh-web-finding__top"><h3>${esc(rowName(row))}</h3><span class="gwh-web-status is-missing">More information required</span></div><ul>${(row.missingQuestions || row.missingInputs || []).map((item) => `<li>${esc(clean(item))}</li>`).join("")}</ul></article>`).join("")}<div class="gwh-web-boundary"><strong>Your next step</strong><p>Return to the GrowWithHR assessment, complete the missing facts and regenerate the report.</p></div></section>` : ""}
                ${watch.length ? `<section class="gwh-web-section"><p class="gwh-web-eyebrow">GROWTH COMPLIANCE RADAR</p><h2>What may change as you grow</h2><p class="gwh-web-lede">A future trigger is a reason to reassess. It is not, by itself, a declaration that a law automatically applies.</p>${watch.map((row) => `<article class="gwh-web-simple"><div class="gwh-web-finding__top"><h3>${esc(rowName(row))}</h3><span class="gwh-web-status is-watch">Watch as you grow</span></div><dl class="gwh-web-trigger"><div><dt>Current position</dt><dd>${esc(clean(row.thresholdResult?.positionText || row.thresholdResult?.label, "Not currently triggered"))}</dd></div><div><dt>Reassessment point</dt><dd>${esc(clean(row.thresholdResult?.triggerText || row.reassessWhen || row.futureTrigger, "Reassess after a relevant company change"))}</dd></div><div><dt>Why</dt><dd>${esc(clean(row.thresholdResult?.explanation || row.whyIncluded))}</dd></div></dl></article>`).join("")}<h3>When to run GrowWithHR again</h3><ul><li>Employee headcount changes materially</li><li>Employee wage or workforce-category information changes</li><li>Contractor arrangements change</li><li>A new State, office or establishment is added</li><li>The establishment activity or operating model changes</li></ul></section>` : ""}
