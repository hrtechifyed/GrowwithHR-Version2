...
(() => {
    ...
    const FLOW_FIX_MARKER =
        "__assessmentFlowFixesInstalled";
    ...
    const fieldValue = (
        ...
        return cleanText(
            field?.value ??
            application?.answers?.[fieldName]
        );
    };

    const setNavigationBusy = (
        application,
        busy
    ) => {
        const button =
    ...
    const installAssessmentFlowFixes = (
        ...
        application.continueFromMoment =
            function continueWithAssessmentGuards() {
                if (navigationLocked) {
                    window.GWHR_LOG(
        ...
        window.GrowWithHRAssessmentFirstSceneFix =
            Object.freeze({
                version: "3.0.0",
                installed: true,
        ...
        window.GrowWithHRAssessmentNavigationGuard =
            Object.freeze({
                version: "1.0.0",
                installed: true,
    ...
    const loadPrivateBetaModules = () => {
        ...
        import(
            "./assessment-v3/compliance-story-presentation.js"
        ).catch((error) => {
            console.error(
    ...
    const loadReportExperienceAndPdf = async () => {
        ...
        const pipeline = (async () => {
            ...
            await loadPdfModule(
                "./pdf-executive-enhancements.js",
                "GrowWithHR: executive PDF enhancements could not load."
            );
            await loadPdfModule(
