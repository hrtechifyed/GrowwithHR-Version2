...
function scenarioSection(doc,state,payload,logoData){
    ...
    heading(doc,state,"Scenario basis and assumptions",2,"12-Month Growth Scenario",logoData);
    labelValue(doc,state,"GrowWithHR rule",`${cleanText(payload.scenario.ruleId||payload.scenario.id)} v${cleanText(payload.scenario.ruleVersion,"Not recorded")}`,"12-Month Growth Scenario",logoData);
    paragraph(doc,state,payload.scenario.ruleBasis,{size:8.3,pageTitle:"12-Month Growth Scenario"},logoData);
    (payload.scenario.assumptions||[]).forEach(assumption=>paragraph(doc,state,assumption,{size:8.3,pageTitle:"12-Month Growth Scenario"},logoData));
