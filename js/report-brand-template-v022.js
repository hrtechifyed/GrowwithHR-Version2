/* GrowWithHR v0.22 shared HRTechify report template */
(() => {
    "use strict";
    ...
    const VERSION = "0.22.2-full-width-hrtechify-template";
    ...
    function createWriter(doc, colours, sectionPages = {}) {
        let y = PAGE.top;
        const split = (value, width = CONTENT_WIDTH) => doc.splitTextToSize(clean(value), width);
        ...
        function label(value, colour = colours.accent) {
            ...
            doc.text(clean(value).toUpperCase(), PAGE.left, y, { maxWidth: CONTENT_WIDTH });
            y += 7;
        }

        function title(value, intro = "") {
            const headingLines = split(value, CONTENT_WIDTH);
            label(value);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(21);
            doc.setTextColor(...colours.heading);
            doc.text(headingLines, PAGE.left, y, { lineHeightFactor: 1.08, maxWidth: CONTENT_WIDTH });
            y += headingLines.length * lineHeight(21, 1.08) + 4;

            if (intro) {
                const introLines = split(intro, CONTENT_WIDTH);
                doc.setFont("helvetica", "normal");
                doc.setFontSize(8.8);
                doc.setTextColor(...colours.muted);
                doc.text(introLines, PAGE.left, y, { lineHeightFactor: 1.28, maxWidth: CONTENT_WIDTH });
        ...
        function infoCard(cardTitle, rows, options = {}) {
            const width = Math.min(CONTENT_WIDTH, Number(options.width || CONTENT_WIDTH));
            ...
            prepared.forEach((row) => {
                ...
                doc.text(row.label.toUpperCase(), PAGE.left + 10, cursor, { maxWidth: width - 20 });
                cursor += 4.5;
                doc.setFont("helvetica", "normal");
                doc.setFontSize(8);
        ...
        function checkItem(value, colour = colours.red) {
            ...
            const lines = split(base.compact(value, 210), innerWidth);
            const height = Math.max(14, lines.length * lineHeight(8.2, 1.25) + 8);
            ensure(height + 3);
            doc.setFillColor(...colours.surface);
            doc.setDrawColor(...colours.line);
            doc.roundedRect(PAGE.left, y, CONTENT_WIDTH, height, 2, 2, "FD");
            ...
            doc.setFontSize(8.2);
            doc.setTextColor(...colours.text);
            doc.text(lines, PAGE.left + 18, y + 7, { lineHeightFactor: 1.25, maxWidth: innerWidth });
            y += height + 3;
        }

        function timelineCard(windowLabel, heading, references, colour) {
            const textWidth = CONTENT_WIDTH - 26;
            const lines = split(base.compact(references, 180), textWidth);
            const height = Math.max(34, 22 + lines.length * lineHeight(8.2, 1.22));
            ensure(height + 4);
            const top = y;
            doc.setFillColor(...colours.surface);
            doc.setDrawColor(...colours.line);
            doc.roundedRect(PAGE.left, top, CONTENT_WIDTH, height, 3, 3, "FD");
            doc.setFillColor(...colour);
            ...
            doc.setFontSize(10);
            doc.setTextColor(...colours.heading);
            doc.text(heading, PAGE.left + 25, top + 11, { maxWidth: textWidth });
            doc.setFont("helvetica", "normal");
            doc.setFontSize(8.2);
            doc.setTextColor(...colours.text);
            doc.text(lines, PAGE.left + 25, top + 20, { lineHeightFactor: 1.22, maxWidth: textWidth });
            y += height + 4;
        }
        ...
        function compactTable(headers, rows, widths) {
            ...
            const safeWidths = widths.map((width) => Number(width || 0) * scale);

            const drawRow = (cells, header = false) => {
                const lines = cells.map((cell, index) => split(base.compact(cell, 120), safeWidths[index] - 4));
                ...
                cells.forEach((_cell, index) => {
                    ...
                    doc.setDrawColor(...colours.line);
                    doc.rect(x, y, safeWidths[index], height, "FD");
                    doc.setFont("helvetica", header ? "bold" : "normal");
                    doc.setFontSize(size);
                    doc.setTextColor(...(header ? colours.heading : colours.text));
                    doc.text(lines[index], x + 2, y + 5, { lineHeightFactor: 1.18, maxWidth: safeWidths[index] - 4 });
                    x += safeWidths[index];
                });
                y += height;
            };
