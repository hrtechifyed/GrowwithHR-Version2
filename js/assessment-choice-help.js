...
(() => {
  ...
  function openGuide(control, title, entry, trigger) {
    ...
    document.dispatchEvent(new CustomEvent('growwithhr:choice-help-opened', { detail: { control, entry, dialog: panel } }));
  }
  function enhance(control) {
    ...
    if (titleElement.tagName === 'LEGEND' || titleElement.contains(control)) titleElement.append(host); else titleElement.insertAdjacentElement('afterend', host);
