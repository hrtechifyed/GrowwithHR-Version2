/* ==========================================================
   ...
   - Loads the founder assessment guided-industry enhancement on assessment pages
   ========================================================== */
(function installGrowWithHRPdfLineLayoutFixes(window, document) {
    ...
    if (!currentPdf || typeof currentPdf.buildAdvisoryPdf !== "function" || !JsPDF) {
        console.warn("GrowWithHR PDF line-layout fixes: the PDF service is unavailable.");
        return;
    }
    ...
    const PAGE = Object.freeze({ width: 210, height: 297, borderInset: 5.5 });
    const THEMES = Object.freeze({
        light: Object.freeze({ page: [255,255,255], heading: [4,28,67], line: [166,181,202], accent: [245,158,11] }),
        dark: Object.freeze({ page: [7,16,31], heading: [255,255,255], line: [61,82,111], accent: [245,158,11] })
    });
    ...
    function fullParagraphText(text) {
        return cleanText(Array.isArray(text) ? text.join(" ") : text);
    }

    function currentFontStyle(doc) {
        try {
            return cleanText(doc.getFont?.().fontStyle || doc.internal?.getFont?.().fontStyle).toLowerCase();
        } catch (_error) {
            return "";
        }
    }
    ...
    function isRunningText(doc, text, options = {}) {
        ...
        const hasMultipleSourceLines = Array.isArray(text) && text.length > 1;
        const wrapsWithinWidth = maximumWidth > MIN_RUNNING_WIDTH && measuredTextWidth(doc, paragraph) > maximumWidth;

        if (maximumWidth <= MIN_RUNNING_WIDTH) return false;
        if (alignment && !["left", "justify"].includes(alignment)) return false;
        if (fontSize < 7 || fontSize > 11) return false;
        if (fontStyle.includes("bold")) return false;
        if (paragraph.split(/\s+/).length < 4) return false;
        if (!hasMultipleSourceLines && alignment !== "justify" && !wrapsWithinWidth) return false;
        return true;
    }
    ...
    function patchTextTarget(target, restorers) {
        ...
        target.text = function fillEachRunningLine(text, x, y, options, transform) {
            const alignment = cleanText(options?.align).toLowerCase();
            const hasPreSplitLines = Array.isArray(text) && text.length > 1;
            ...
            if (hasPreSplitLines && alignment === "justify") {
                const paragraphOptions = { ...(options || {}) };
                delete paragraphOptions.align;
                return originalText.call(this, text, x, y, paragraphOptions, transform);
            }

            return originalText.call(this, text, x, y, options, transform);
        };

        restorers.push(() => { target.text = originalText; });
    }

    async function buildWithFullLineText(build) {
        const restorers = [];
        const targets = Array.from(new Set([JsPDF.API, JsPDF.prototype].filter((target) => target && typeof target.text === "function")));
        targets.forEach((target) => patchTextTarget(target, restorers));
        try {
            return await build();
        } finally {
    ...
    function decorateEndPage(doc, themeName, logoDataUrl) {
        ...
        if (logoDataUrl) {
            try {
                doc.addImage(logoDataUrl, "PNG", centreX - logoSize / 2, logoY, logoSize, logoSize, `end-report-logo-${themeName}`, "FAST");
            } catch (_error) {}
        }
        ...
        doc.text("End of Report", centreX, 166, { align: "center" });
    }
    ...
    function decorateBuiltResult(result, logoDataUrl) {
        if (Array.isArray(result?.pdfs)) {
            const pdfs = result.pdfs.map((variant) => {
                decorateEndPage(variant.document, variant.theme, logoDataUrl);
            ...
            return {
                ...
                sizeBytes: pdfs.reduce((sum, item) => sum + (Number(item.sizeBytes) || 0), 0),
        ...
        decorateEndPage(result?.document, result?.theme, logoDataUrl);
        return serialisePdfResult(result);
    }

    async function buildAdvisoryPdf(payload = {}) {
        const [result, logoDataUrl] = await Promise.all([
            buildWithFullLineText(() => originalBuild(payload)),
            loadLogoDataUrl()
        ]);
        return decorateBuiltResult(result, logoDataUrl);
    }
    ...
    window.GrowWithHRPDFLineLayoutReady = Promise.resolve(enhancedPdf);
...
if (document.body?.classList.contains("analyze-company-page")) {
    import("./assessment-founder-ux-v1.js").catch((error) => {
        console.error("GrowWithHR guided assessment UX could not load.", error);
    });
}
