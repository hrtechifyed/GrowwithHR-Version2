...
export function WizardForm() {
  ...
  const submitCompany = () => {
    ...
    persistence.holdCurrentTabDraft({ id: crypto.randomUUID(), currentStep: 'contact', companyData: store.companyData, leadData: store.leadData, report: store.report ?? undefined });
  ...
  if (store.currentStep === 'contact') return <SceneTransition sceneKey="contact" title="Contact details for this tab only"><LeadCaptureForm onSuccess={() => store.setStep('report')} /></SceneTransition>;
  ...
  return (
    <SceneTransition sceneKey="company" title="Tell us enough to personalize the advisory">
      <form className="gwhr-wizard" aria-label="Executive company assessment" onSubmit={(e) => { e.preventDefault(); submitCompany(); }}>
        ...
        <p role="note">{persistence.noStorageNotice}</p>
        <div className="gwhr-actions"><button className="primary" type="submit">Generate personalized advisory</button><button className="secondary" type="button" onClick={() => store.reset()}>Start fresh</button></div>
