...
export function LeadCaptureForm({ onSuccess }: LeadCaptureFormProps) {
  ...
  return (
    <form className="gwhr-lead" aria-label="Contact details for advisory delivery" onSubmit={(e) => { e.preventDefault(); void submit(); }}>
      <style>{`.gwhr-lead{display:grid;gap:1rem;font-family:"Inter","Segoe UI",sans-serif}.gwhr-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem}.gwhr-field{display:grid;gap:.35rem}.gwhr-field input{min-height:44px;border:1px solid #c9d7ee;border-radius:14px;padding:.75rem 1rem;font:inherit}.gwhr-error{color:#b42318;font-size:.9rem}.gwhr-submit{min-height:48px;border:0;border-radius:999px;background:#102033;color:white;padding:.85rem 1.2rem;font-weight:700}.gwhr-success{border-radius:16px;background:#ecfdf3;color:#067647;padding:1rem}@media(max-width:720px){.gwhr-fields{grid-template-columns:1fr}}`}</style>
      <p>Where should we send your advisory? In this client-side build these fields are held only in the open tab and are not stored, emailed, exported to a CRM, or visible in any HRTechify admin dashboard.</p>
      ...
      <label><input type="checkbox" checked={Boolean(leadData.consentToEmail)} onChange={(e) => updateLeadData({ consentToEmail: e.target.checked })} /> I understand this client-side build does not store or send my contact details; if I leave or refresh, I will need to start fresh.</label>
      ...
      {sent ? <div className="gwhr-success" role="status">Contact details captured for this open tab only. No storage or remote delivery occurred.</div> : null}
    </form>
  );
}
