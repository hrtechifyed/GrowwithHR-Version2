"use strict";
const base=require("./server-legal-explanation-router-wave5f.js");
const {buildAllLawsPrivateBetaRegistry}=require("./server-all-laws-private-beta-wave5g.js");
const {createRunnableAllLawsFeatureSpecifications}=require("./server-all-laws-rule-catalogs-wave5g.js");
const {loadGovernedLegalCatalogs}=require("./server-legal-rag-catalogs.js");
const SHARED_ROUTER_VERSION="1.17.0";
...
function statusPayload(registry,catalogSnapshot,retrievalMode){return Object.freeze({...base.statusPayload(registry,catalogSnapshot,retrievalMode),routerVersion:SHARED_ROUTER_VERSION,limitations:[
 ...
 "Waves 1–5G remain needs-legal-review and emit specialist-review or more-information-needed, not legal certification, standing-orders applicability, worker-threshold, certification, dispute, termination, strike/lock-out, retrenchment, closure or remedy decisions.",
 "Six remaining profiles are runnable through conservative governance-fallback catalogues.",
 "Wave 5G processes bounded Industrial Relations source route, Code/amendment/commencement, Central Rules, Model Standing Orders, Maharashtra draft-rule, transition, threshold-source, classification, certification-process, authority and escalation statuses plus controlled references only.",
 "Retrieval and provider output cannot treat draft State rules as final, infer worker counts or establishment classification, adopt or certify standing orders, select a forum, adjudicate disputes or alter deterministic outcomes."
 ]});}
