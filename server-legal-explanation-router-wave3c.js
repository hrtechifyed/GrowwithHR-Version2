...
const base=require("./server-legal-explanation-router-wave3b.js");
const {buildAllLawsPrivateBetaRegistry}=require("./server-all-laws-private-beta-wave3c.js");
const {createRunnableAllLawsFeatureSpecifications}=require("./server-all-laws-rule-catalogs-wave3c.js");
const {loadGovernedLegalCatalogs}=require("./server-legal-rag-catalogs.js");
const SHARED_ROUTER_VERSION="1.6.0";
...
function statusPayload(registry,catalogSnapshot,retrievalMode){
 return Object.freeze({...base.statusPayload(registry,catalogSnapshot,retrievalMode),routerVersion:SHARED_ROUTER_VERSION,
  limitations:[
   ...
   "Waves 1–3C remain needs-legal-review and emit specialist-review or more-information-needed, not legal certification, exemption decisions, certificate validation or individual membership decisions.",
   "Twenty-eight remaining profiles are runnable through conservative governance-fallback catalogues.",
   "Wave 3C processes organisation-level exemption and international-worker controls and evidence references only.",
   "Retrieval and provider output cannot create facts, infer a person or country, validate documents or alter deterministic outcomes."
  ]});
}
...
function createSharedLegalExplanationRequestHandler(options={}){
 ...
 const delegated=base.createSharedLegalExplanationRequestHandler({...options,profileRegistry,featureSpecifications,catalogSnapshot,catalogs:options.catalogs||catalogSnapshot.catalogs});
 const retrievalMode=text(options.retrievalMode||process.env.LEGAL_RAG_RETRIEVAL_MODE||"lexical").toLowerCase();
 return function handleWave3cLegalExplanationRequest(request,response){
  const requestPath=text(request.url).split("?")[0];
  if(requestPath===base.STATUS_ROUTE&&request.method==="GET"){writeStatus(response,statusPayload(profileRegistry,catalogSnapshot,retrievalMode));return true;}
