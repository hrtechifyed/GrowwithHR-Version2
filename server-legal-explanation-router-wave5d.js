"use strict";
const base=require("./server-legal-explanation-router-wave5c.js");
const {buildAllLawsPrivateBetaRegistry}=require("./server-all-laws-private-beta-wave5d.js");
const {createRunnableAllLawsFeatureSpecifications}=require("./server-all-laws-rule-catalogs-wave5d.js");
const {loadGovernedLegalCatalogs}=require("./server-legal-rag-catalogs.js");
const SHARED_ROUTER_VERSION="1.14.0";
...
function statusPayload(registry,catalogSnapshot,retrievalMode){return Object.freeze({...base.statusPayload(registry,catalogSnapshot,retrievalMode),routerVersion:SHARED_ROUTER_VERSION,limitations:[
 "Seven POSH, ten Maternity Benefit, twelve EPF/EPS/EDLI, fifteen ESI, one Appropriate Government, one Maharashtra Shops, one Code on Wages and one Gratuity profile use feature-specific deterministic rules and governed catalogues.",
 "Waves 1–5D remain needs-legal-review and emit specialist-review or more-information-needed, not legal certification, Gratuity coverage, eligibility, amount, claim, appeal, recovery or remedy decisions.",
 "Nine remaining profiles are runnable through conservative governance-fallback catalogues.",
 "Wave 5D processes organisation-level Chapter V, First Schedule, Rules, commencement, transition, establishment-classification, workforce-category-source, authority-process and escalation statuses plus controlled references only.",
 "Retrieval and provider output cannot classify an establishment, count employees, infer service or worker facts, calculate gratuity, determine an entitlement or alter deterministic outcomes."
 ]});}
