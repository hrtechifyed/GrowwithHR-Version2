...
const base = require("./server-all-laws-private-beta-wave3a.js");
const {
    EPF_WAVE3B_CATALOG_ID,
    EPF_WAVE3B_CATALOG_PATH,
    EPF_WAVE3B_FEATURE_IDS,
    EPF_WAVE3B_PROFILE_DEFINITIONS
...
function buildAllLawsPrivateBetaRegistry(options = {}) {
    ...
    const definitionByFeature = new Map(
        EPF_WAVE3B_PROFILE_DEFINITIONS.map((item) => [item.featureId, item])
    );
    const featureIds = new Set(EPF_WAVE3B_FEATURE_IDS);
    ...
    registry.profiles = array(registry.profiles).map((profileValue) => {
        ...
        return {
            ...
            catalogId: EPF_WAVE3B_CATALOG_ID,
    ...
    registry.limitations = [
        ...
        "Wave 3B does not calculate contributions, select a legally applicable rate branch, decide individual membership, pension or insurance benefits, or resolve transition and savings questions.",
